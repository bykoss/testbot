...
# PATCH GLOBAL - “by Exagonal” en TODOS los embeds autom
...
if token_env:
cfg[“token”] = token_env
if cfg.get(“token”) in (””, “TU_TOKEN_AQU
“, None):
log.critical(“No se encontr
token.”)
sys.exit(1)
return cfg
...
ROLES_STAFF_CFG = CONFIG.get(“roles_staff”, [”? Administraci
n”, “?? Moderador”])
...
await log_antinuke(guild, “?? Ban Revertido”,
f”**V
ctima:** {user.mention} (`{user.id}`)\n**Baneado por:** {autor.mention}\n**Acci
n:** Desbaneado autom
ticamente”,
color=0x00FF88)
except Exception as e:
...
pass
await log_antinuke(role.guild, “?? Rol Restaurado”,
...
await log_antinuke(role.guild, “?? Rol No Autorizado Eliminado”,
...
m = role.guild.get_member(autor.id) or await role.guild.fetch_member(autor.id)
if m:
await ejecutar_castigo(role.guild, m, f”Creaci
n masiva de roles ({count})”)
await log_antinuke(role.guild, “? Creaci
n Masiva de Roles”,
f”**Usuario:** {autor.mention}\n**Roles creados:** {count}\n**Acci
n:** `{cfg['accion']}`”)
except Exception as e:
...
name=channel.name,
overwrites=overwrites,
reason=f”[AntiNuke] Restaurando categor
a eliminada por {autor}”
)
else:
nuevo_canal = await channel.guild.create_text_channel(
name=channel.name,
...
pass
await log_antinuke(channel.guild, “?? Canal Restaurado”,
...
await log_antinuke(channel.guild, “?? Canal No Autorizado Eliminado”,
f”**Canal:** `#{nombre}`\n**Creado por:** {autor.mention}\n**Acci
n:** Eliminado autom
ticamente”,
color=0xFF8800)
...
m = channel.guild.get_member(autor.id) or await channel.guild.fetch_member(autor.id)
if m:
await ejecutar_castigo(channel.guild, m, f”Creaci
n masiva de canales ({count})”)
await log_antinuke(channel.guild, “? Creaci
n Masiva de Canales”,
...
if m:
await ejecutar_castigo(channel.guild, m, f”Creaci
n masiva de webhooks ({count})”)
await log_antinuke(channel.guild, “?? Webhooks Masivos”,
f”**Usuario:** {autor.mention}\n**Webhooks:** {count}\n**Acci
n:** `{cfg['accion']}`”)
except Exception as e:
...
adido por:** {autor.mention}”, color=0xFFAA00)
return
except Exception:
pass
ar = cfg.get(“antiraid”, {})
...
await message.channel.send(f”? {message.author.mention} No se permiten links aqu
.”, delete_after=5)
...
if r:
try:
await member.add_roles(r, reason=“Verificaci
except Exception:
pass
if rol_no:
...
try:
await member.remove_roles(r, reason=“Verificaci
except Exception:
pass

# ??????????????????????????????????????????????????????????????

# ?? COMANDOS ANTINUKE (solo Owner del AntiNuke)
...
embed = discord.Embed(title=”?? AntiNuke - Panel Completo”, color=0x00FF88 if cfg[“activo”] else 0xFF0000)
embed.add_field(name=“Estado”,   value=estado,                            inline=True)
embed.add_field(name=“Acci
n”,   value=cfg.get(“accion”, “ban”).upper(),  inline=True)
...
p = PREFIX
embed = discord.Embed(title=”?? AntiNuke - Comandos”, color=0x00FF88)
embed.add_field(name=”?? General”,
value=(
f”`{p}antinuke` - Panel de estado\n”
f”`{p}an_activar` / `{p}an_desactivar` - Activar/desactivar\n”
f”`{p}an_accion <ban|kick|quitar_roles>` - Acci
n al detectar\n”
f”`{p}an_limite <tipo> <n>` - Cambiar l
mite\n”
f”`{p}an_ventana <segundos>` - Ventana de tiempo\n”
f”`{p}an_whitelist @user` - A
adir/quitar de whitelist\n”
...
embed.add_field(name=”? AntiBot / ? Verificaci
value=(
f”`{p}an_antibot_on` / `{p}an_antibot_off` - Bloquear bots no autorizados\n”
f”`{p}an_ver_setup #canal @rol_verificado @rol_no_verificado` - Setup verificaci
n\n”
f”`{p}an_ver_on` / `{p}an_ver_off` - Activar/desactivar verificaci
), inline=False)
embed.add_field(name=”?? Warns”,
value=(
f”`{p}warn @user <raz n>` - Advertir usuario\n”
...
await ctx.send(”?? AntiNuke **desactivado**. El servidor queda sin protecci
n.”)
...
embed = discord.Embed(
title=f”?? Whitelist - {ctx.guild.name}”,
...
embed = discord.Embed(
title=”?? Quitado de Whitelist”,
description=f”{member.mention} ya **no est
** en la whitelist de **{ctx.guild.name}**.”,
...
title=”? A
adido a Whitelist”,
description=f”{member.mention} ahora est
en la whitelist de **{ctx.guild.name}**.\nEl AntiNuke lo ignorar
en este servidor.”,
...
return await ctx.send(”?? Canal de logs **eliminado**.”)
...
embed.add_field(name=“L
mite”,   value=f”{ar.get(‘joins_limite’,10)} joins”, inline=True)
embed.add_field(name=“Ventana”,  value=f”{ar.get(‘joins_ventana’,10)}s”,     inline=True)
embed.add_field(name=“Acci
n”,   value=ar.get(“accion”,“kick”).upper(),      inline=True)
await ctx.send(embed=embed)
...
await ctx.send(”? AntiBot **activado**. Bots no autorizados ser
n expulsados.”)
...
await ctx.send(f”? Verificaci
n configurada en {canal.mention}.”)
...
async def warn(ctx, member: discord.Member, *, razon: str = “Sin raz
n”):
...
guardar_warns(data)
total = len(data[uid])
embed = discord.Embed(title=”?? Advertencia”, color=discord.Color.orange())
embed.add_field(name=”? Usuario”,  value=member.mention,    inline=True)
embed.add_field(name=”? Raz
n”,    value=razon,              inline=True)
...
await ctx.send(f”? {member.mention} fue baneado autom
ticamente por alcanzar 5 warns.”)
elif total >= 3:
import datetime as dt
...
if uid in parejas:
return await ctx.send(f”? Ya est
s casado/a. Usa `{PREFIX}divorcio` primero.”)
if mid in parejas:
return await ctx.send(f”? {member.mention} ya est
casado/a.”)
propuestas_pendientes[member.id] = ctx.author.id
embed = discord.Embed(title=”?
Propuesta!”, description=f”{ctx.author.mention} le propone a {member.mention}\nUsa `{PREFIX}aceptar` o `{PREFIX}rechazar` en 60s.”, color=discord.Color.pink())
await ctx.send(embed=embed)
await asyncio.sleep(60)
if propuestas_pendientes.get(member.id) == ctx.author.id:
propuestas_pendientes.pop(member.id, None)
await ctx.send(f”? La propuesta a {member.mention} expir
...
embed = discord.Embed(title=”?
Se casaron!”, description=f”{ctx.author.mention} y <@{autor_id}>
Felicidades! ?”, color=discord.Color.gold())
await ctx.send(embed=embed)
...
return await ctx.send(”? Sin propuesta pendiente.”)
autor_id = propuestas_pendientes.pop(ctx.author.id)
await ctx.send(f”? {ctx.author.mention} rechaz
a <@{autor_id}>.”)
...
return await ctx.send(”? No est
s casado/a.”)
ex_id = parejas.pop(uid); parejas.pop(str(ex_id), None)
guardar_parejas(parejas)
embed = discord.Embed(title=”? Divorcio”, description=f”{ctx.author.mention} se divorci
de <@{ex_id}>.”, color=discord.Color.red())
await ctx.send(embed=embed)
...
if uid not in parejas:
return await ctx.send(f”? {member.display_name} no est
casado/a.”)
embed = discord.Embed(title=”? Estado Civil”, description=f”{member.mention} est
con <@{parejas[uid]}> ?”, color=discord.Color.pink())
await ctx.send(embed=embed)
...
embed = discord.Embed(title=”???
Adopci
n!”, description=f”{ctx.author.mention} adopt
a {member.mention} ?”, color=discord.Color.green())
await ctx.send(embed=embed)
...
“cancer”: (”?”,“21 jun - 22 jul”,“Intuitivo, protector y emp
tico.”),
“leo”: (”?”,“23 jul - 22 ago”,“Carism
tico, generoso y l
der nato.”),
“virgo”: (”?”,“23 ago - 22 sep”,“Anal
tico, detallista y perfeccionista.”),
“libra”: (”?”,“23 sep - 22 oct”,“Justo, diplom
tico y encantador.”),
“escorpio”: (”?”,“23 oct - 21 nov”,“Intenso, misterioso y poderoso.”),
“sagitario”: (”?”,“22 nov - 21 dic”,“Aventurero, optimista y filos
fico.”),
...
“piscis”: (”?”,“19 feb - 20 mar”,“Compasivo, art
stico y so
ador.”),
PREDICCIONES = [
“? Un encuentro inesperado cambiar
tu d
a.”,
“? El dinero fluye si act
as con confianza.”,
“?? El amor est
s cerca de lo que crees.”,
...
ximo.”,
“? Buen d
a para empezar algo nuevo.”,
“? Una amistad te sorprender
positivamente.”,
“? Tu energ
a es imparable,
sala bien.”,
“? La noche traer
claridad a tus dudas.”,
...
embed.add_field(name=”? Predicci
n”, value=random.choice(PREDICCIONES), inline=False)
await ctx.send(embed=embed)
TIPOS_PERSONALIDAD = [
(”? Alma de Fuego”,“Intenso/a, apasionado/a y siempre vas al frente.”),
(”? Esp
ritu del Agua”,“Tranquilo/a, profundo/a y adaptable.”),
(”?? Mente del Viento”,“Veloz, creativo/a y lleno/a de ideas.”),
(”? Coraz
n de Tierra”,“Estable, confiable y roca para todos.”),
(”? Rayo de Energ
a”,“Energ
a inagotable que contagia a todos.”),
(”? Alma Lunar”,“Misterioso/a, intuitivo/a y emocional.”),
(”?? Esp
ritu Solar”,“Irradias positividad y alegr
a.”),
(”?? Mente de Hielo”,“Fr
o/a bajo presi
n y muy anal
...
random.seed()
if pct >= 80:   estado = “?
Almas gemelas!”; color = discord.Color.pink()
...
PREGUNTAS_TRIVIA = [
{“p”:”
ntos lados tiene un hex
gono?”,“r”:“6”,“ops”:[“4”,“5”,“6”,“8”]},
{“p”:”
Capital de Jap
n?”,“r”:“tokio”,“ops”:[“osaka”,“tokio”,“beijing”,“seul”]},
{“p”:”
Planetas en el sistema solar?”,“r”:“8”,“ops”:[“7”,“8”,“9”,“10”]},
{“p”:”
o del hombre en la luna?”,“r”:“1969”,“ops”:[“1965”,“1969”,“1971”,“1973”]},
{“p”:”
Elemento m
s abundante en el universo?”,“r”:“hidrogeno”,“ops”:[“oxigeno”,“helio”,“hidrogeno”,“carbono”]},
{“p”:”
Colores del arco
ris?”,“r”:“7”,“ops”:[“5”,“6”,“7”,“8”]},
{“p”:”
Animal m
pido del mundo?”,“r”:“guepardo”,“ops”:[“leon”,“guepardo”,“tigre”,“aguila”]},
{“p”:”
ano m
s grande?”,“r”:“pacifico”,“ops”:[“atlantico”,“indico”,“pacifico”,“artico”]},
{“p”:”
Huesos del cuerpo humano adulto?”,“r”:“206”,“ops”:[“180”,“196”,“206”,“220”]},
{“p”:”
s grande del mundo?”,“r”:“rusia”,“ops”:[“canada”,“china”,“rusia”,“eeuu”]},
{“p”:”
Planeta rojo?”,“r”:“marte”,“ops”:[“venus”,“marte”,“jupiter”,“saturno”]},
{“p”:”
nto es 15 x 15?”,“r”:“225”,“ops”:[“200”,“215”,“225”,“250”]},
{“p”:”
Metal m
s caro del mundo?”,“r”:“rodio”,“ops”:[“oro”,“platino”,“rodio”,“iridio”]},
{“p”:”
Continente de Brasil?”,“r”:“america del sur”,“ops”:[“africa”,“america central”,“america del sur”,“europa”]},
{“p”:”
Segundos en una hora?”,“r”:“3600”,“ops”:[“1200”,“3000”,“3600”,“4800”]},
...
await ctx.send(f”? {user.mention} fall
. Era **{p[‘r’].capitalize()}**.”)
...
embed = discord.Embed(title=”? Adivina el N
mero”,
description=f”N
mero entre **1 y {maximo}**. Tienes **{max_intentos} intentos**.”, color=discord.Color.blurple())
await ctx.send(embed=embed)
...
{msg.author.mention} acert
! Era **{numero}** en {intentos} intento(s)!”)
...
No voy a rendirme!”,”
Cree en ti mismo!”,”
Hokage!”,“El dolor te hace m
s fuerte.”],
“goku”:[”
Soy un Saiyan de la Tierra!”,”
Kamehameha!”,“No puedo perder. Hay gente que me importa.”],
“luffy”:[”
el Rey de los Piratas!”,”
Libertad!”,”
Un hombre que no protege a sus amigos no vale nada!”],
“zoro”:[“Nada me sucede hasta que yo digo que algo me sucede.”,”
Nunca perder
de nuevo!”,“Solo hay un camino: adelante.”],
“eren”:[“Si no luchas, no puedes ganar.”,“La libertad es lo
nico que he querido.”],
“levi”:[“La
nica forma de encontrar la respuesta es elegir y no arrepentirte.”,“Tus camaradas conf
an en ti.”],
“light”:[“Soy el nuevo dios de este mundo.”,“El que gana tiene raz
n.”],
“itachi”:[“Eres d
bil porque te falta odio.”,“El perd
n es la base de la paz.”,“Siempre ser
tu hermano mayor.”],
...
@commands.check(es_admin)
async def lock(ctx, canal: discord.TextChannel = None, *, razon: str = “Sin raz
n”):
...
@commands.check(es_admin)
async def unlock(ctx, canal: discord.TextChannel = None, *, razon: str = “Sin raz
n”):
canal = canal or ctx.channel
...
@commands.check(es_admin)
async def lockall(ctx, *, razon: str = “Sin raz
n”):
...
@commands.check(es_admin)
async def unlockall(ctx, *, razon: str = “Sin raz
n”):
...
await ctx.send(f”?? {canal.mention} **oculto**.”)
...
await canal.set_permissions(ctx.guild.default_role, overwrite=ow)
await ctx.send(f”?? {canal.mention} **visible**.”)
...
if canal != ctx.channel: await ctx.send(f”?? **#{nombre}** eliminado.”)
...
return await ctx.send(f”? No puedo dar **{rol.name}** porque est
por encima de mi rol en la jerarqu
a.”)
if rol in member.roles:
return await ctx.send(f”?? {member.mention} ya tiene **{rol.name}**.”)
try:
...
await ctx.send(”? Sin permisos para dar ese rol. Verifica la jerarqu
a del bot.”)
...
rol = r; break
if not rol:
return await ctx.send(f”? No encontr
`{nombre_rol}`.”)
if rol >= ctx.guild.me.top_role:
return await ctx.send(f”? No puedo gestionar **{rol.name}** (jerarqu
a).”)
if rol not in member.roles:
return await ctx.send(f”?? {member.mention} no tiene **{rol.name}**.”)
try:
...
return await ctx.send(”? Color inv
lido. Usa `#RRGGBB`.”)
...
await ctx.send(f”?? **{nombre_rol}** eliminado.”)
except discord.Forbidden:
...
@commands.check(es_admin)
async def ban_cmd(ctx, member: discord.Member, *, razon: str = “Sin raz
n”):
...
if not objetivo: return await ctx.send(f”? No encontr
`{usuario}` en los bans.”)
...
@commands.check(es_admin)
async def kick_cmd(ctx, member: discord.Member, *, razon: str = “Sin raz
n”):
...
@commands.check(es_admin)
async def mute_cmd(ctx, member: discord.Member, minutos: int = 10, *, razon: str = “Sin raz
n”):
...
embed.add_field(name=”? Usuario”,   value=member.mention,     inline=True)
embed.add_field(name=”? Duraci
n”,  value=f”{minutos} min”,   inline=True)
embed.add_field(name=”? Raz
n”,     value=razon,              inline=True)
embed.add_field(name=”? Por”,       value=ctx.author.mention, inline=True)
await ctx.send(embed=embed)
...
msg = await ctx.send(f”?? **{len(borrados)-1}** mensajes borrados.”)
...
msg = await ctx.send(f”?? **{len(borrados)}** mensajes de {member.mention} borrados.”)
await asyncio.sleep(3); await msg.delete()
...
o”,    value=g.owner.mention,   inline=True)
...
embed.add_field(name=”? Nivel verificaci
n”, value=str(g.verification_level), inline=True)
...
await interaction.response.send_message(”?? Se quitar
n **TODOS** los roles.”, ephemeral=True)
return
coincidencias = [
...
f”? No encontr
ning
n rol con `{self.input.value}`. Intenta de nuevo.”, ephemeral=True
)
return
...
await interaction.response.send_message(
f”? Encontr
**{len(coincidencias)}** roles. Selecciona uno:”,
view=view_sel,
ephemeral=True
)
...
btn_todos = discord.ui.Button(label=”?? Quitar todos los roles”, style=discord.ButtonStyle.secondary, row=1)
...
await interaction.response.send_message(”? Solo quien ejecut
el comando puede usar esto.”, ephemeral=True)
return False
return True
...
self.rol_quitar_id = “ALL”
await interaction.response.send_message(”?? Se quitar
n **TODOS** los roles al confirmar.”, ephemeral=True)
...
f”?? **Quitar todos** - elimina todos los roles del usuario\n\n”
...
if not rol_dar_id:
return await ctx.send(”? No hay rol configurado para dar. Selecciona uno en el men
rol_dar = ctx.guild.get_role(rol_dar_id)
if not rol_dar:
return await ctx.send(”? No encontr
el rol a dar.”)
roles_quitados = []
roles_fallidos = []
...
return await ctx.send(f”? No pude asignar **{rol_dar.name}**. Sube el rol del bot en la jerarqu
a.”)
...
if roles_quitados:
embed_ok.add_field(
name=f”?? Roles quitados ({len(roles_quitados)})”,
value=”, “.join(f”`{r.name}`” for r in roles_quitados),
inline=False
)
if roles_fallidos:
embed_ok.add_field(
name=”?? No se pudieron quitar”,
...
if len(nuevo) > 3: return await ctx.send(”? M
x 3 caracteres.”)
...
embed.add_field(name=”? En l
nea”, value=en_linea, inline=True)
...
embed.add_field(name=”?? Prefijo”,   value=f”`{bot.command_prefix}`”,inline=True)
await ctx.send(embed=embed)
...
embed = discord.Embed(title=”? Invitar”, description=f”
[Clic aqu
]({url})”, color=discord.Color.blurple())
await ctx.send(embed=embed)
...
embed   = discord.Embed(title=f”?? {ciudad.title()}”, color=discord.Color.blue())
embed.add_field(name=”?? Temp”,      value=f”{actual[‘temp_C’]}
C”,       inline=True)
embed.add_field(name=”? Sensaci
n”, value=f”{actual[‘FeelsLikeC’]}
C”,   inline=True)
...
embed.add_field(name=”?? Estado”,    value=actual[‘weatherDesc’][0][‘value’], inline=True)
await ctx.send(embed=embed)
...
embed = discord.Embed(title=”? Traducci
n”, color=discord.Color.teal())
...
await ctx.send(embed=embed)
except ZeroDivisionError: await ctx.send(”? Divisi
n entre cero.”)
except Exception: await ctx.send(”? Expresi
n inv
lida.”)
...
await ctx.send(f”? Te recordar
en **{cantidad} {nombres[unidad]}**.”)
...
if uid in cumples: return await ctx.send(f”? Tu cumplea
os: **{cumples[uid]}**.”)
return await ctx.send(”? No tienes cumplea
os. Usa `!cumple DD/MM`.”)
try:
...
if uid not in cumples: return await ctx.send(f”? {member.display_name} sin cumplea
os.”)
...
embed.add_field(name=”? Faltan”, value=f”**{dias}** d
as”, inline=True)
await ctx.send(embed=embed)
@bot.command(name=“cumples_lista”, aliases=[“lista_cumples”])
async def cumples_lista(ctx):
cumples = cargar_cumples()
if not cumples: return await ctx.send(”? Nadie ha registrado cumplea
os.”)
...
lista.sort()
embed = discord.Embed(title=”? Pr
ximos Cumplea
os”, color=discord.Color.gold())
for dias, uid, fecha in lista[:10]:
member = ctx.guild.get_member(int(uid))
nombre = member.display_name if member else f”<@{uid}>”
embed.add_field(name=f”? {nombre}”, value=f”**{fecha}** - en {dias} d
as”, inline=False)
await ctx.send(embed=embed)
...
respuestas = [
“? S
, definitivamente.”,”? Todo indica que s
.”,”? Sin duda.”,
“? No est
claro.”,”? Conc
ntrate y pregunta de nuevo.”,
“? No cuentes con ello.”,”? Mi respuesta es no.”,”? Definitivamente no.”
]
embed = discord.Embed(title=”? Bola M
gica”, color=discord.Color.dark_purple())
...
resultado = “?
Ganaste!”; color = discord.Color.green()
else: resultado = “?
Perdiste!”; color = discord.Color.red()
embed = discord.Embed(title=”? RPS”, description=resultado, color=color)
embed.add_field(name=“T
“,  value=emojis[eleccion],     inline=True)
...
acertijos = [
(“Tengo ciudades, pero no hay casas. Tengo monta
as, pero no hay
rboles. Tengo agua, pero no hay peces.
soy?”, “Un mapa”),
(“Cuanto m
s me seques, m
s mojado te quedas.
soy?”, “Una toalla”),
(“Tengo manos pero no puedo aplaudir.
soy?”, “Un reloj”),
(“Soy ligero como una pluma, pero ni el hombre m
s fuerte puede sostenerme m
s de unos minutos.
soy?”, “El aliento”),
(“Tengo un coraz
n que no late, tengo una boca que no habla.
soy?”, “Una alcachofa”),
(“Siempre delante de ti, pero no se puede ver.
soy?”, “El futuro”),
]
...
FRASES_MOTIVACION = [
“El
xito no es definitivo, el fracaso no es fatal. - Churchill”,
“El
nico modo de hacer un gran trabajo es amar lo que haces. - Jobs”,
“La vida es 10% lo que te sucede y 90% c
mo reaccionas. - Swindoll”,
“El futuro pertenece a quienes creen en la belleza de sus sue
os. - Roosevelt”,
“S
el cambio que quieres ver en el mundo. - Gandhi”,
“No esperes oportunidades extraordinarias. Aprovecha las ordinarias.”,
“Cree en ti mismo y todo lo dem
s vendr
solo.”,
CHISTES = [
“
Por qu
los p
jaros vuelan al sur? Porque caminar es muy lejos ?”,
“
le dijo el 0 al 8? Bonito cintur
n ?”,
“
Por qu
el libro de matem
ticas estaba triste? Porque ten
a muchos problemas ?”,
“
hace una abeja en el gimnasio?
Zum-ba! ?”,
“
Por qu
los esqueletos no pelean? No tienen agallas ?”,
@bot.command(name=“frase”, aliases=[“motivacion”,“quote”])
async def frase_random(ctx):
embed = discord.Embed(title=”? Frase del d
a”, description=f”*{random.choice(FRASES_MOTIVACION)}*”, color=discord.Color.teal())
await ctx.send(embed=embed)
...
await ctx.send(”? No pude obtener un meme. Intenta m
s tarde.”)
@bot.command(name=“rng”, aliases=[“random”,“aleatorio”])
async def rng(ctx, minimo: int = 1, maximo: int = 100):
if minimo >= maximo: return await ctx.send(”? El m
nimo debe ser menor que el m
ximo.”)
resultado = random.randint(minimo, maximo)
embed = discord.Embed(title=”? N
mero Aleatorio”, color=discord.Color.blurple())
...
if len(opciones) > 9: return await ctx.send(”? M
ximo 9 opciones.”)
...
“cry”:      {“emoji”:”?”,“gif_tag”:“cry”,    “msg”:”{a} est
llorando ?”,            “boton”:“Consolar ?”},
...
if interaction.user.id != self.target.id:
return await interaction.response.send_message(”? No es para ti.”, ephemeral=True)
await interaction.response.send_message(f”? **{self.target.display_name}** rechaz
a **{self.autor.display_name}**.”)
self.stop()
...
description=f”Prefix: `{p}` - Bot multiprop
sito con moderaci
n, AntiNuke, juegos y m
color=discord.Color.blurple()
)
...
f”`{p}sugerencia <txt>` `{p}reporte @user <raz n>` `{p}invitar`”
), inline=False)
embed.add_field(name=”?? AntiNuke (Owner)”,
value=(
f”`{p}antinuke` - Panel completo\n”
f”`{p}an_ayuda` - Todos los comandos AntiNuke\n”
f”AntiRaid | AntiLinks | AntiSpam | AntiBot | Verificaci
), inline=False)
embed.add_field(name=”?? Warns (Staff)”,
value=f”`{p}warn @u <raz n>` `{p}warns [@u]` `{p}clearwarns @u` `{p}delwarn @u <n>`”,
inline=False)
embed.add_field(name=”? Moderaci
n (Admin)”,
value=(
...
await ctx.send(f”? Argumento inv
lido. Usa `{PREFIX}ayuda`.”)
